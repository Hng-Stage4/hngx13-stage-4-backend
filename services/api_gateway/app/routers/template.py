...
async def get_template(template_id: str, current_user: dict = Depends(get_current_user)):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{TEMPLATE_SERVICE_URL}/api/templates/{template_id}")
        return response.json()
...
async def get_template_versions(template_id: str, current_user: dict = Depends(get_current_user)):
    """Proxy template versions to template service"""
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{TEMPLATE_SERVICE_URL}/api/templates/{template_id}/versions")
        return response.json()
